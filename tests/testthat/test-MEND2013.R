...
test_that("MEND_pools", {
  ...
  state <- c(P = P,  M = M,  Q = Q,  B = B,  D = D,  EP = EP,  EM = EM,  IC = IC,  Tot = Tot)
  expect_error(MEND_pools(t = t, state = state, parms = MEND_params, flux_function =  'not a list'),
               "flux_function is not a function", fixed = TRUE)
  expect_error(MEND_pools(t = t, state = state[1:2], parms = MEND_params, flux_function = function(){}),
               'missing states: Q,  B,  D,  EP,  EM,  IC,  Tot')
  expect_error(MEND_pools(t = t, state = state, parms = MEND_params[ , 1:2], flux_function =  function(){}),
               "parms does not have all of these name(s): 'parameter', 'description', 'units', 'value'", fixed = TRUE)
  expect_error(MEND_pools(t = t, state = state, parms = MEND_params[1:2, ], flux_function =  function(){}),
               "parms missing values for:  I.p, g.d, f.d, I.d", fixed = TRUE)
  expect_error(MEND_pools(t = t, state = rev(state), parms = MEND_params, flux_function =  MEND_fluxes),
               'state pools must be in the following order: P,  M,  Q,  B,  D,  EP,  EM,  IC,  Tot')


  xx <- MEND_pools(state = state, parms = MEND_params)
  ...
  yy <- MEND_pools(state = state, parms = MEND_params, flux_function = MEND_fluxes)
  # Length of the output should equal the number of states (pools of carbon) and is some number.
...
test_that("deafult MEND values", {
  ...
  expect_true(is.vector(MEND_initalState))
  expect_equal(sum(MEND_initalState[names(MEND_initalState) != 'Tot']), MEND_initalState[['Tot']])

  expect_true(data.table::is.data.table(MEND_params))
